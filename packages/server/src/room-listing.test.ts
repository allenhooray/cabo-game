...
describe("public room listings", () => {
  const source = (overrides: Record<string, unknown> = {}) => ({
    roomId: "room-1",
    locked: false,
    metadata: { visibility: "public", roomName: "Friday", phase: "LOBBY", targetScore: 100, playerCount: 1, maxClients: 4 },
    ...overrides,
  });
  ...
  it("includes waiting, full, started, and locked public rooms with joinability", () => {
    ...
    expect(toPublicRoomListing(source({ metadata: { ...source().metadata, playerCount: 4 } }))).toMatchObject({ isFull: true, isStarted: false, canJoin: false });
