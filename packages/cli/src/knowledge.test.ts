...
describe("private card knowledge", () => {
  ...
  it("handles reveal-before-state ordering at the start of the next round", () => {
    const state = applyReveal(createKnowledge(1), { memoryMode: "assisted", ownerId: "alice", round: 2, reason: "initial", position: 1, card: { id: "a", label: "2♥", rank: 2 } }, 1, "ROUND_RESULT");
    expect(state.round).toBe(2);
    expect(state.slots[0]?.label).toBe("2♥");
    const firstRound = applyReveal(createKnowledge(), { memoryMode: "assisted", ownerId: "alice", round: 1, reason: "initial", position: 2, card: { id: "b", label: "9♣", rank: 9 } }, 0, "LOBBY");
  ...
  it("updates replacements and discard draws only after their public event", () => {
    ...
    state = applyReveal(state, { memoryMode: "assisted", ownerId: "alice", round: 1, reason: "draw", card: { id: "b", label: "3♠", rank: 3 } }, 1, "DRAWN");
