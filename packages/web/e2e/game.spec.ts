...
test("two isolated players create, join, start, and reconnect", async ({ browser }) => {
  ...
  await alice.goto("/");
  await alice.getByLabel("Player name").fill("Alice");
  ...
  await bob.goto("/");
  await bob.getByLabel("Player name").fill("Bob");
...
test("private room requires its six-digit password", async ({ browser }) => {
  ...
  await host.goto("/");
  await host.getByLabel("Player name").fill("Host");
  ...
  await guest.goto("/");
  await guest.getByLabel("Player name").fill("Guest");
