...
function state(overrides: Partial<CaboStateLike> = {}): CaboStateLike {
  return { memoryMode: "assisted", turnDurationSeconds: 60, deadlineAt: 0, serverTime: 0, revision: 1, roomName: "Alice's room", phase: "LOBBY", round: 0, targetScore: 100, currentPlayerId: "", caboCallerId: "", drawSource: "", mismatchPenaltyCardPending: false, discardLabel: "", discardRank: -1, deckCount: 0, players: new Map([["a", alice], ["b", bob]]), winners: [], ...overrides };
}

describe("legalActions", () => {
  it("permits skipping only an exhausted final turn", () => {
    expect(legalActions(state({ phase: "FINAL_TURNS", currentPlayerId: "a", caboCallerId: "b" }), "a")).toEqual([{ type: "skip" }]);
    expect(legalActions(state({ phase: "FINAL_TURNS", currentPlayerId: "a", caboCallerId: "b", deckCount: 1 }), "a")).not.toContainEqual({ type: "skip" });
  });
  it("covers lobby, draw, replacement, and powers", () => {
    ...
    expect(legalActions(state({ phase: "POWER_PENDING", currentPlayerId: "a", discardRank: 11 }), "a")).toContainEqual({ type: "swap", targetPlayerId: "b", ownPosition: 2, targetPosition: 2 });
