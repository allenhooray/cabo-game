...
- 建房默认经典模式（`classic`）：初始两张牌显示 5 秒，窥牌和错配公开牌显示 3.2 秒，不自动保存牌位记忆；未处理的抽牌持续显示。辅助模式（`assisted`）保留已知牌，并由服务端在重连时恢复。
- 每个有效游戏步骤默认限时 60 秒，也可选择 30、90 秒或不限时。服务端超时自动安全完成当前回合；聊天和断线不会暂停或延长时限。
- 局末全员就绪立即继续，否则 20 秒后自动开下一轮（不限时房间同样适用）。
- 极端情况下两处牌堆均耗尽，超时会宣告 Cabo；无牌可抽的最后回合允许跳过，以免卡局。
- Web 的 Share 可复制房间码或邀请链接。链接包含房间码和目标服务器，加入前会明确展示地址；复制失败会提供手动复制文本。
- 多牌替换必须选择所选牌中的一个落点；J/Q 可交换双方不同位置。Cabo 确认提示严格最低、平分失败、失败手牌分 +5；辅助模式额外显示已知小计。

CLI 示例：`create public 100 --mode classic --timer 60 --name "Friends"`；交换：`swap PLAYER OWN_POS TARGET_POS`。Agent 使用协议 v7，建房必须发送 `memoryMode` 与 `turnDurationSeconds`，四端需协调升级。
...
- 7/8 看自己，9/10 看他人，11/12 与他人交换任意位置；仅从牌堆抽出后直接弃掉时触发。
